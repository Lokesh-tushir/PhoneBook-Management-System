import json  # We need this module to work with JSON files
...
def load_contacts():
    ...
    try:
        ...
        # If the file is empty or corrupt, return an empty list
        print("Warning: Could not read contact file. Starting fresh.")
        return []

def save_contacts(contacts):
    """Saves the entire contact list back to the JSON file."""
    # 'w' means write (it will overwrite the old file)
    with open(FILE_NAME, 'w') as f:
        # json.dump writes the list to the file, indent=2 makes it readable
        json.dump(contacts, f, indent=2)

def find_contact(contacts, name):
    """Helper function to find the first contact that matches a name."""
...
# This standard line runs the main() function when you start the script
